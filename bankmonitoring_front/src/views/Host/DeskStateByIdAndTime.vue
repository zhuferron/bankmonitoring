  ...
  <div>
    ...
    <div>
      <el-table :data="dataShow" style="width: 100%">
        ...
        <el-table-column prop="deskStateId" label="磁盘状态" align="center">
        </el-table-column>
        <el-table-column prop="fileSystem" label="盘符类型" align="center">
        </el-table-column>
        <el-table-column label="使用百分比" align="center">
          <template scope="scope">
            <el-progress :text-inside="true":percentage="getNumber(scope.row.usePer)" :show-text='true':status="setColor(getNumber(scope.row.usePer))":stroke-width="22"></el-progress>
            <div slot="reference" class="name-wrapper">
              <el-tag size="medium">可用：{{ scope.row.avail }}</el-tag>
              &#12288;
              <el-tag size="medium">已用：{{ scope.row.used }}</el-tag>
              &#12288;
              <el-tag size="medium">总共：{{ scope.row.size }}</el-tag>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column  label="操作" align="center">
          <el-button type="primary" @click="goDiskUsage">磁盘详细状况</el-button>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods:{
    goDiskUsage(){
      this.$router.push("/diskUsage")
    },
    async getTableData() {
      const {data: res1} = await this.$http.get('http://192.168.43.73:9002/getDeskStateByTime/'+ '2021-06-25_14:00:00')  //访问后台
      if(res1.code==200){
