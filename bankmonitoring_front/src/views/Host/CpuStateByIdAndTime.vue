  ...
  <div>
    ...
    <div>
      <el-table :data="dataShow" style="width: 100%">
        ...
        <el-table-column prop="cpuStateId" label="CPU状态ID" align="center">
        </el-table-column>
        <el-table-column prop="speed" label="CPU速度" align="center">
        </el-table-column>
        <el-table-column label="CPY使用率" align="center">
          <template scope="scope">
            ...
            <div slot="reference" class="name-wrapper">
              <el-tag size="medium">使用中CPU个数：{{ scope.row.num}}</el-tag>
              &#12288;
              <el-tag size="medium">CPU使用率：{{ scope.row.usePer}}</el-tag>
              &#12288;
              <el-tag size="medium">CPU当前空闲率：{{ scope.row.idle }}</el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="createTime" label="创建时间" align="center">
        </el-table-column>
        <el-table-column label="CPU等待率" align="center">
          <template scope="scope">
            <div>
              <el-progress type="circle" :percentage="getNumber(scope.row.iowait)":status="setColor(getNumber(scope.row.iowait))" :stroke-width="10">
              </el-progress>
            </div>
          <div>
            <el-tag size="medium">
              CPU当等待率：{{ scope.row.iowait }}
            </el-tag>
          </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods:{
    async getTableData() {
      const {data: res1} = await this.$http.get('http://192.168.43.73:9002/getCpuStateByTime/'+ '2021-06-25_14:00:00')  //访问后台
      if(res1.code==200){
